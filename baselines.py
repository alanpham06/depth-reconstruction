...
def nearest_fill(inputs: torch.Tensor, ref: torch.Tensor, chunk: int = 8192) -> torch.Tensor:
	...
	for b in range(batch):
		...
		seeds = pixels[mask]
		nearest = torch.cat([torch.cdist(block, seeds).argmin(dim=1) for block in pixels.split(chunk)])
