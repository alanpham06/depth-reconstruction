...
def assemble_input(sparse_depth: torch.Tensor, sparse_mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
class SparseDepthDataset(Dataset):
	...
	def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
		...
		if self.augment:
			...
			if self.noise_std > 0:
				sparse_depth = sparse_depth + self.noise_std * torch.randn_like(sparse_depth)
