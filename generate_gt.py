...
def parse_args() -> argparse.Namespace:
	...
	parser.add_argument("--points", type=int, default=5000, help="surface samples drawn per view")
	parser.add_argument("--edge-fraction", type=float, default=0.25, help="share of samples near sharp edges")
	parser.add_argument("--corner-fraction", type=float, default=0.05, help="share of samples near corners")
	parser.add_argument("--band", type=float, default=0.03, help="width scale of the edge/corner bands")
...
def save_preview(name: str, data: dict[str, torch.Tensor], path: Path) -> None:
	...
	for view in range(n_views):
		...
		axes[2, view].imshow(sparse, cmap="viridis", vmin=float(depth[mask].min()), vmax=float(depth[mask].max()))
