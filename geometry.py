...
def uv_sphere_mesh(n_lat: int = 34, n_lon: int = 128, radius: float = 1.0) -> Mesh:
	...
	faces = []
	first = ring(0)
	faces.append(torch.stack([torch.full_like(first, north), first, first.roll(-1)], dim=-1))
	...
	last = ring(n_rings - 1)
	faces.append(torch.stack([torch.full_like(last, south), last.roll(-1), last], dim=-1))
...
def icosphere_mesh(subdivisions: int = 4, radius: float = 1.0) -> Mesh:
	...
	for _ in range(subdivisions):
		...
		unique_edges, inverse = torch.unique(edges.sort(dim=-1).values, dim=0, return_inverse=True)
		midpoints = vertices[unique_edges].mean(dim=1)
		midpoints = midpoints / midpoints.norm(dim=-1, keepdim=True)
		mid = (inverse + vertices.shape[0]).reshape(3, -1)  # mid[k] = midpoint of edge k per face
		m01, m12, m20 = mid
...
def cube_mesh(n_side: int = 27, half_extent: float = 1.0) -> Mesh:
	...
	i, j = torch.meshgrid(torch.arange(n_side - 1), torch.arange(n_side - 1), indexing="ij")
	p00 = (i * n_side + j).reshape(-1)
	p10, p01, p11 = p00 + n_side, p00 + 1, p00 + n_side + 1
	cell_faces = torch.cat([torch.stack([p00, p10, p11], -1), torch.stack([p00, p11, p01], -1)])
	...
	for axis in range(3):
		...
		for sign in (-1.0, 1.0):
			...
			face = cell_faces if torch.dot(torch.linalg.cross(e_a, e_b), normal) > 0 else cell_faces.flip(-1)
