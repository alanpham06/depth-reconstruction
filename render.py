...
def look_at(eye, target=(0.0, 0.0, 0.0), up=(0.0, 0.0, 1.0)) -> torch.Tensor:
	...
	eye, target, up = (torch.as_tensor(x, dtype=torch.float64) for x in (eye, target, up))
	forward = target - eye
...
def mesh_intersector(mesh: Mesh, chunk: int = 1024, eps: float = 1e-9) -> Intersector:
	...
	def intersect(origin: torch.Tensor, dirs: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
		...
		for start in range(0, dirs.shape[0], chunk):
			...
			hit = valid & (bu >= -eps) & (bv >= -eps) & (bu + bv <= 1.0 + eps) & (t > eps)
			...
			bw = 1.0 - bu - bv
			face = faces[local]
			normal = bw[:, None] * n0[face] + bu[:, None] * n1[face] + bv[:, None] * n2[face]
			normal = normal / normal.norm(dim=-1, keepdim=True).clamp_min(1e-12)
			t_out[start : start + chunk] = t_min
			n_out[start : start + chunk] = torch.where(torch.isfinite(t_min)[:, None], normal, 0.0)
		return t_out, n_out
...
def render(camera: Camera, intersect: Intersector, tile: int = 32) -> dict[str, torch.Tensor]:
	...
	tile_id = (pixel // camera.width // tile) * camera.width + (pixel % camera.width) // tile
