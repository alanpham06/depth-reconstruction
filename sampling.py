...
def find_sharp_features(mesh: Mesh, angle_deg: float = 30.0) -> SharpFeatures:
	...
	cos[pair_ok] = (face_normals[face_of[start[pair_ok]]] * face_normals[face_of[second[pair_ok]]]).sum(-1)
	...
	return SharpFeatures(edge_faces, edge_slots, corner_faces, corner_slots, int(is_corner.sum()))


def _interpolate_normals(mesh: Mesh, faces: torch.Tensor, bary: torch.Tensor) -> torch.Tensor:
	normals = (mesh.vertex_normals[mesh.faces[faces]] * bary[..., None]).sum(dim=1)
	return normals / normals.norm(dim=-1, keepdim=True)


def _samples(mesh: Mesh, faces: torch.Tensor, bary: torch.Tensor, kind: int) -> SurfaceSamples:
...
def sample_uniform(mesh: Mesh, n: int, generator: torch.Generator | None = None) -> SurfaceSamples:
	"""Area-weighted face choice plus uniform barycentrics (sqrt trick)."""
	faces = torch.multinomial(triangle_areas(mesh), n, replacement=True, generator=generator)
...
def _band_offset(n: int, band: float, generator: torch.Generator | None) -> torch.Tensor:
...
def sample_edges(
	mesh: Mesh, features: SharpFeatures, n: int, band: float, generator: torch.Generator | None = None
) -> SurfaceSamples:
	...
	a, b, c = tri[rows, i], tri[rows, (i + 1) % 3], tri[rows, (i + 2) % 3]
...
def sample_corners(
	...
	direction_length = (mix[:, None] * eb[pick] + (1.0 - mix[:, None]) * ec[pick]).norm(dim=-1)
...
def project_points(points: torch.Tensor, camera: Camera) -> tuple[torch.Tensor, torch.Tensor]:
...
def visible_points(
	points: torch.Tensor, camera: Camera, intersect: Intersector, rel_tol: float = 1e-6, tile: int = 32
) -> torch.Tensor:
	...
	in_frame = (z > 0) & (uv[:, 0] >= 0) & (uv[:, 0] < camera.width) & (uv[:, 1] >= 0) & (uv[:, 1] < camera.height)
	...
	for block, block_dirs in zip(candidates.split(counts.tolist()), dirs.split(counts.tolist())):
