...
from losses import completion_loss, depth_l1, depth_metrics, gradient_loss, summarize_metrics
from unet import UNet


def _plane(height: int = 32, width: int = 32, slope: float = 0.01, offset: float = 3.0) -> torch.Tensor:
...
def test_assemble_input_centers_on_sparse_mean():
	...
	batch, refs = assemble_input(torch.stack([depth, 2 * depth]), torch.stack([mask, mask]))
...
def test_gradient_loss_detects_wrong_slope_and_rounded_crease():
	...
	assert gradient_loss(smooth, crease, valid) > gradient_loss(crease, crease, valid) + 1e-3
