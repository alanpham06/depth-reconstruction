...
def test_spheres_are_closed_outward_and_on_surface():
    for mesh in (uv_sphere_mesh(), icosphere_mesh()):
        ...
        assert torch.allclose(
            mesh.vertices.norm(dim=-1), torch.ones(1, dtype=torch.float64)
        )
        assert torch.allclose(mesh.vertex_normals, mesh.vertices)
        centroids = mesh.vertices[mesh.faces].mean(dim=1)
        assert ((mesh.face_normals() * centroids).sum(-1) > 0).all(), (
            "faces must wind outward"
        )
...
def test_cube_is_six_plane_grids():
    ...
    assert torch.allclose(
        mesh.vertices.abs().amax(dim=-1), torch.ones(1, dtype=torch.float64)
    )
...
def test_intrinsics_and_extrinsics_project_consistently():
    ...
    assert torch.allclose(
        target_cam[:3], torch.tensor([0.0, 0.0, 3.0], dtype=torch.float64)
    )
...
def _check_render_invariants(
    view: dict[str, torch.Tensor], camera, grazing_tol_deg: float = 0.0
) -> None:
    mask, normals = view["mask"], view["normals"]
    assert mask.any()
    assert (view["depth"][~mask] == 0).all() and (normals[~mask] == 0).all()
    assert torch.allclose(
        normals[mask].norm(dim=-1), torch.ones(1, dtype=torch.float64)
    )
    ...
    assert (
        (normals * dirs_cam).sum(-1)[mask] < math.sin(math.radians(grazing_tol_deg))
    ).all()
...
def test_cube_mesh_render_matches_analytic_box():
    ...
    for camera in CAMERAS:
        mesh_view, exact_view = (
            render(camera, intersect_mesh),
            render(camera, intersect_exact),
        )
        ...
        assert torch.allclose(
            points.abs().amax(dim=-1), torch.ones(1, dtype=torch.float64), atol=1e-9
        )
...
def test_sphere_mesh_render_matches_analytic_sphere():
    ...
    for mesh in (uv_sphere_mesh(), icosphere_mesh()):
        ...
        for camera in CAMERAS:
            ...
            assert (mesh_view["depth"] - exact_view["depth"])[
                both
            ].abs().median() < 3e-3
            ...
            assert (
                _angle_deg(mesh_view["normals"][mesh_view["mask"]], true_normals).max()
                < 1e-4
            )
...
def test_sphere_normals_are_continuous():
    ...
    def jumps(view):
        ...
        return _angle_deg(normals[1:, 1:], normals[1:, :-1]), _angle_deg(
            normals[1:, 1:], normals[:-1, 1:]
        )

    # Stay off the silhouette, where the true normal turns arbitrarily fast per pixel.
    facing = (
        mesh_view["mask"] & exact_view["mask"] & (exact_view["normals"][..., 2] < -0.5)
    )
...
def _cube_edge_distance(points: torch.Tensor) -> torch.Tensor:
    ...
    free[
        torch.arange(points.shape[0]), on_face
    ] = -math.inf  # ignore the face's own axis
...
def test_sharp_features():
    ...
    assert torch.allclose(
        _cube_edge_distance(torch.cat([a, b])), torch.zeros(1, dtype=torch.float64)
    )
...
def test_uniform_sampling_is_area_weighted():
    ...
    assert torch.allclose(
        sphere.normals.norm(dim=-1), torch.ones(1, dtype=torch.float64)
    )

    cube = sample_uniform(cube_mesh(), n, generator)
    assert torch.allclose(
        cube.points.abs().amax(dim=-1), torch.ones(1, dtype=torch.float64)
    )
    face = cube.points.abs().argmax(dim=-1) * 2 + (
        cube.points.gather(1, cube.points.abs().argmax(-1, keepdim=True)).squeeze(1) > 0
    )
    ...
    d = 0.1
    assert (
        abs((_cube_edge_distance(cube.points) < d).float().mean() - (1 - (1 - d) ** 2))
        < 0.005
    )
...
def test_smooth_meshes_sample_uniformly():
    samples = sample_surface(
        icosphere_mesh(), 10_000, generator=torch.Generator().manual_seed(0)
    )
...
def test_cube_sampling_is_denser_at_edges_and_corners():
    ...
    n = 200_000
    samples = sample_surface(
        mesh, n, band=0.03, generator=torch.Generator().manual_seed(0)
    )
    points = samples.points
    assert torch.allclose(points.abs().amax(dim=-1), torch.ones(1, dtype=torch.float64))
    assert torch.bincount(samples.kind).tolist() == [
        n * 70 // 100,
        n * 25 // 100,
        n * 5 // 100,
    ]
    ...
    edge_band_area = (
        12 * 2 * (2 * d)
    )  # 12 edges x 2 faces, strip of length 2 and width d
    ...
    corner_area = (
        8 * 3 * (math.pi * d**2 / 4)
    )  # quarter disc on each of 3 faces per corner
...
def test_visibility_is_exact_for_convex_shapes():
    ...
    for mesh in (icosphere_mesh(), cube_mesh()):
        samples = sample_surface(
            mesh, 20_000, generator=torch.Generator().manual_seed(1)
        )
...
def test_sparse_depth_matches_dense_ground_truth():
    for mesh in (icosphere_mesh(), cube_mesh()):
        ...
        for camera in CAMERAS:
            ...
            samples = sample_surface(
                mesh, 20_000, generator=torch.Generator().manual_seed(2)
            )
            depth, mask = sparse_depth_map(samples.points, camera, intersect)
            assert (
                depth.shape == mask.shape == (camera.height, camera.width)
            ...
            assert (depth[check] >= lo[check] - slack).all() and (
                depth[check] <= hi[check] + slack
            ).all()
