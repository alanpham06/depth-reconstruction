...
class UNet(nn.Module):
	def __init__(self, in_channels: int = 2, out_channels: int = 1, base: int = 32, depth: int = 4):
		super().__init__()
		widths = [base * 2**level for level in range(depth + 1)]
		self.stem = DoubleConv(in_channels, widths[0])
		self.down = nn.ModuleList(DoubleConv(widths[i], widths[i + 1]) for i in range(depth))
		self.up = nn.ModuleList(Up(widths[i + 1], widths[i]) for i in reversed(range(depth)))
